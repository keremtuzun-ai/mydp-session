...
export function ResetRequestForm() {
  ...
  return (
    <div className="space-y-6">
      ...
      {sent ? (
        <form action={codeAction} className="space-y-4 rounded-md border bg-muted/40 p-4" noValidate>
          <p className="text-sm font-medium">Have a code from the email?</p>
          <input type="hidden" name="email" value={email} />
          <Field label="Recovery code" htmlFor="token">
            <Input id="token" name="token" inputMode="numeric" pattern="\d{6}" maxLength={6} className="font-mono tracking-[0.4em]" />
          </Field>
...
export function NewPasswordForm() {
  ...
  return (
    <form action={action} className="space-y-4" noValidate>
      ...
      <FormError message={state && !state.ok ? state.error : null} />
      <SubmitButton className="w-full" variant="gold" pendingText="Saving…">
        Set new password
      </SubmitButton>
    </form>
  );
